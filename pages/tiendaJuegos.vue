   <template>
    <div>
      ...
      <div class="generos">
        <CardTipo v-for="(game, index) in genreGames" :key="index" :botonTexto="firstOptions[index].text"
          :genreId="firstOptions[index].id" :gameImage="game.background_image" :gameTitle="game.name" />
      </div>
      <HeaderSubtitulo HeadingText="Tipos de juego" :showPagar="false" :showPuntos="false" :options="secondOptions" />
      <div class="generos">
        <CardTipo v-for="(game, index) in typeGames" :key="index" :botonTexto="secondOptions[index].text"
          :genreId="secondOptions[index].id" :gameImage="game.background_image" :gameTitle="game.name" />
      </div>
    </div>
  </template>
